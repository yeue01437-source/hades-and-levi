...
function parseDiceData(rawData) {
    try {
        ...
        function findAllDiceNumbers(obj, depth = 0) {
            if (depth > 20) return []; // Chống đệ quy quá sâu
        ...
        if (allDiceNumbers.length >= 3) {
            // Lấy session/phiên từ API gốc nếu có
            let sessionFromApi = null;
            
            // Hàm tìm session trong object
            function findSession(obj, depth = 0) {
                if (depth > 20) return null;
                if (!obj || typeof obj !== 'object') return null;
                
                // Tìm key chứa 'session' hoặc 'phien' hoặc 'id'
                for (const key in obj) {
            ...
            // Nếu không tìm thấy session, dùng timestamp
            if (!sessionFromApi) {
                // Tìm timestamp
    ...
    try {
        ...
        if (newHash === lastJsonHash) {
            ...
            if (diceData) {
                ...
                const session = {
                    phien: diceData.phien, // PHIÊN TỪ API GỐC
                ...
                if (history.length >= MAX_SESSIONS) {
                    console.log('\n' + '='.repeat(60));
                    console.log('🎯 ĐÃ ĐỦ 50 PHIÊN!');
                    console.log(`📡 API hiện tại: ${API_URL}`);
                    console.log(`💾 File lịch sử: ${HISTORY_FILE}`);
                    console.log('='.repeat(60));
                    console.log('✅ Vẫn tiếp tục theo dõi, phiên cũ nhất sẽ bị xóa khi có phiên mới...\n');
                }
            } else {
                console.log(`[${timeStr}] ⚠️  JSON thay đổi nhưng không parse được dữ liệu`);
                console.log(`   Raw data: ${rawData.substring(0, 150)}...`);
            }
        }
    } catch (error) {
